...
public class Turtle extends Enemy {
    ...
    public Turtle(PlayScreen screen, float x, float y) {
        ...
        setBounds(getX(), getY(), 16 / MarioBrosServer.PPM, 24 / MarioBrosServer.PPM);
    ...
    protected void defineEnemy() {
        ...
        fdef.filter.categoryBits = MarioBrosServer.ENEMY_BIT;
        fdef.filter.maskBits = MarioBrosServer.GROUND_BIT |
                MarioBrosServer.COIN_BIT |
                MarioBrosServer.BRICK_BIT |
                MarioBrosServer.ENEMY_BIT |
                MarioBrosServer.OBJECT_BIT |
                MarioBrosServer.MARIO_BIT;
        ...
        vertice[0] = new Vector2(-5, 8).scl(1 / MarioBrosServer.PPM);
        vertice[1] = new Vector2(5, 8).scl(1 / MarioBrosServer.PPM);
        vertice[2] = new Vector2(-3, 3).scl(1 / MarioBrosServer.PPM);
        vertice[3] = new Vector2(3, 3).scl(1 / MarioBrosServer.PPM);
        head.set(vertice);

        fdef.shape = head;
        fdef.restitution = 1.8f;
        fdef.filter.categoryBits = MarioBrosServer.ENEMY_HEAD_BIT;
        b2body.createFixture(fdef).setUserData(this);
    }
    ...
    public void update(float dt) {
        ...
        setPosition(b2body.getPosition().x - getWidth() / 2, b2body.getPosition().y - 8 /MarioBrosServer.PPM);
